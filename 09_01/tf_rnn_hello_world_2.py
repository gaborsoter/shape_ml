...
# Network Parameters
n_input = 2  # input is sin(x), a scalar
n_steps = 25  # timesteps
n_hidden = 128  # hidden layer num of features
n_outputs = 4  # output is a series of sin(x+...)
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters or loss_value > target_loss:
        _, batch_x, __, batch_y, ___ = generate_sample(f=None, t0=None, batch_size=batch_size,
    ...
    for i in range(1, n_tests+1):
        ...
        t, y, next_t, expected_y, y_plot = generate_sample(f=i, t0=None, samples=n_steps, predict=1, ninputs = n_input, noutputs = n_outputs)
        ...
        y_plot = y_plot.squeeze()

        plt.plot(t, y_plot, color='black')
        plt.plot(np.append(t[-1], next_t), np.append(y_plot[:,0][-1], prediction[0]), color='red')
        plt.plot(np.append(t[-1], next_t), np.append(y_plot[:,1][-1], prediction[1]), color='red')
        plt.plot(np.append(t[-1], next_t), np.append(y_plot[:,2][-1], prediction[2]), color='red')
        plt.plot(np.append(t[-1], next_t), np.append(y_plot[:,3][-1], prediction[3]), color='red')
        plt.ylim([-1, 1])
        plt.xlabel('time [t]')
        plt.ylabel('signal')
